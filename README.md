...
>
> **The full system is a private build that runs the pipeline across multiple investors.** It ranks their trade ideas on one cross-investor leaderboard, then adds AI-written deep dives on the top ideas and a market sentiment dashboard. The leaderboard is a seventh stage, purely deterministic, that averages investors' latest conviction score per company. Under the hood, it runs one scoped subagent per transcript, and a token-spend dashboard measures cost per stage. On the trust layer, the private build adds a deterministic validator that checks every published figure against the real build, and an extraction record that is never rewritten.
>
